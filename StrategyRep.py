...
import pickle as pk
from OrderParam import get_ensemble_n
import os
...
class PredictorEngine:
    ...
    def VolatilityRegime(self , vol_period=10):
        ...
        features['GAP'] = 100 * abs((self.data['open'] - self.data['close'].shift(1)) / self.data['close'].shift(1))
        features['candle_range'] = (self.data['high'] - self.data['low']).rolling(window=vol_period).mean()
    ...
    def MEAN_REVERT(self, lookback_1, lookback_2, window, normal_window, lags):
