def OrderParam(strategy_name,signal):
    ...
    if strategy_name =='TREND_EMA':
        ...
        if signal == 1:
            p1 = {'opt':'PE', 'step':-6,'transtype':'BUY','Qty':15}
            p2 = {'opt':'PE', 'step':-2,'transtype':'SELL','Qty':15}
            OrderPar = {'p1':p1, 'p2':p2}

        elif signal == -1:
            p1 = {'opt': 'CE', 'step': 6, 'transtype': 'BUY', 'Qty': 15}
            p2 = {'opt': 'CE', 'step': 2, 'transtype': 'SELL', 'Qty': 15}
            OrderPar = {'p1':p1, 'p2':p2}
