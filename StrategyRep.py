...
class PredictorEngine:
    ...
    def load_model(self):
        self.params ,self.VolThres , ensemble = get_ensemble_n(self.strategy_name)
    ...
    def Normalization(self , features , normal_window=10 , skip_col: list = [] , normalization=True ,
                      winsorization=False) :
    ...
    def GetMarketRegime(self) :
        daily_change=self.data['close'].pct_change()
        ...
        return pd.concat([HurstRegime , pd.DataFrame(REGIME_FEAT)] , axis=1)
    ...
    def MomTrading(self , window , lookback , lags=5 , normal_window=10 , X_col=None) :
        ...
        normalized_features=self.Normalization(features[X_col] , normal_window , [] , True)
